...
async def walk_pattern_test(event_loop):
    ...
    for i in range( 1, number_of_samples ):
        walk_reference_states = walker.update_once( tdelta=0.7 )
        await tag.run_once( reference_position=walk_reference_states, tdelta=0.7, binding_key="telemetry" )
        tag_states = tag.get_states()

        # update states
        if ENABLE_PLOTTING:
            x_ref_pos[i] = walk_reference_states["x_ref_pos"]
            y_ref_pos[i] = walk_reference_states["y_ref_pos"]
            z_ref_pos[i] = walk_reference_states["z_ref_pos"]
            x_outlier_pos[i] = tag_states["x_outlier_pos"]
            y_outlier_pos[i] = tag_states["y_outlier_pos"]
            z_outlier_pos[i] = tag_states["z_outlier_pos"]
            input_sample[i] = i
